...
function EditUsers() {
    ...
    useEffect(() => {
        // console.log(props);
        setValue1(props.username);
        setValue2(props.fullName);
        setValue3(props.regNo);
        setValue5(props.roleId);
    },[]);
    ...
    const handleSubmit = async (event) => {
        ...
        if (form.checkValidity() === false) {
            ...
            sendData(props.userId,formdata);
            handleClick();
            setValidated();
        }
    ...
    const sendData = (id,data) => {
        console.log(data);
        axios.put(`http://3.229.95.193:8080/users/${id}`, data)
            .then(response => {
                // Handle success
